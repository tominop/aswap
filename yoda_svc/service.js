 ...
 var lmx = require("./token") // address and ABI of YODA smart contract in Youdex
 ...
 app.get("/lmx/api/:token", (req, res) => {
     ...
     tokenContract = Lmx3.eth.contract(lmx.abi).at(lmx.adrress) // YODA token smart contract in YODAx
     dExContract = Lmx3.eth.contract(dex.abi).at(dex.address) //  Dex smart contract in Youdex
 ...
 app.get("/lmx/startdex/:data", (req, res) => {
     ...
     const valueL = req.params.data.valueL //  value of YODA tokens for pledge
         /*        var myCallData = dexContract.openDEx.getData(alice.ethAddrs, plasmoid.ethAddrs, req.params.data.valueA, req.params.data.valueB, req.params.data.valueL)  // Data for Ethereum transaction DEx smart contract exec
                 ...
                 Lmx3.eth.sendRawTransaction('0x' + serializedTx.toString('hex'))    //  send Tx in YODAx */
     res.header("Access-Control-Allow-Origin", "*")
