...
app.get("/YODA/startDEX/:data", function(req, res, next) {
    ...
    makeYoudexTx('bob', dex.address, 0, myCallData, next, res);
});


//  Route - inDepo function 
app.get("/YODA/inDepo/:data", function(req, res, next) {
    const order = parseInt(req.params.data),
        myCallData = DExContract.inDepo.getData(order);
    makeYoudexTx('plasmoid', dex.address, 0, myCallData, next, res);
})

//  Route - outDepo function 
app.get("/YODA/outDepo/:data", function(req, res, next) {
    const order = parseInt(req.params.data),
